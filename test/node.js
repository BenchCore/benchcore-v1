...
node.bench = require('./bench-js');
node.bench.crypto.setNetworkVersion(network.pubKeyHash);
...
node.normalizer = 100000000; // Use this to convert BEX amount to normal value
...
// Random BEX amount
node.Bench = Math.floor(Math.random() * (100000 * 100000000)) + 1;
...
// Returns random BEX amount
node.randomBex = function () {
...
node.randomAccount = function () {
	...
	account.publicKey = node.bench.crypto.getKeys(account.password, network).publicKey;
	account.address = node.bench.crypto.getAddress(account.publicKey, network.pubKeyHash);
