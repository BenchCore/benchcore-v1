...
function sendBex (account, i, done) {
	var randomBex = node.randomBex();
	...
	node.put('/api/transactions/', {
		...
		if (res.body.success && i != null) {
			accounts[i].balance = randomBex / node.normalizer;
		}
		done();
	});
}

function sendBexFromMultisigAccount (amount, recipient, done) {
	node.put('/api/transactions/', {
...
before(function (done) {
	sendBex(multisigAccount, null, done);
});
...
describe('PUT /api/api/transactions/', function () {
	...
	it('when group transaction is pending should be ok', function (done) {
		sendBexFromMultisigAccount(100000000, node.gAccount.address, function (err, transactionId) {
			node.onNewBlock(function (err) {
