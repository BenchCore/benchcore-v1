	...
	async.whilst(
		function () {
			...
			async.eachSeries(params.delegates, function (delegate, eachCb) {
				var transaction = node.bench.vote.createVote(params.passphrase, [params.action + delegate]);
...
function registerDelegate (account, done) {
	...
	var transaction = node.bench.delegate.createDelegate(account.password, account.username);
...
describe('POST /peer/transactions', function () {
	...
	it('using undefined transaction.asset', function (done) {
		var transaction = node.bench.vote.createVote(account.password, ['+' + delegate]);
	...
	it('voting for a delegate and then removing again within same block should fail', function (done) {
		node.onNewBlock(function (err) {
			...
			postVote(transaction, function (err, res) {
				...
				var transaction2 = node.bench.vote.createVote(account.password, ['-' + delegate]);
	...
	it('voting for 33 delegates at once should be ok', function (done) {
		node.onNewBlock(function (err) {
			var transaction = node.bench.vote.createVote(account.password, delegates.slice(0, 33).map(function (delegate) {
				return '+' + delegate;
			}));
	...
	it('voting for 34 delegates at once should fail', function (done) {
		node.onNewBlock(function (err) {
			var transaction = node.bench.vote.createVote(account.password, delegates.slice(0, 34).map(function (delegate) {
				return '+' + delegate;
			}));
	...
	it('removing votes from 2 delegates at once should fail', function (done) {
		node.onNewBlock(function (err) {
			var transaction = node.bench.vote.createVote(account.password, delegates.slice(0, 2).map(function (delegate) {
				return '-' + delegate;
			}));
...
describe('POST /peer/transactions after registering a new delegate', function () {
	...
	it('voting for self should be ok', function (done) {
		var transaction = node.bench.vote.createVote(account.password, ['+' + account.publicKey]);
	...
	it('exceeding maximum of 1 votes within same block should fail', function (done) {
		async.series([
			...
			function (seriesCb) {
				...
				var transaction = node.bench.vote.createVote(account.password, slicedDelegates.map(function (delegate) {
					return '+' + delegate;
				}));
