...
if(appConfig.network){
	...
	appConfig.network = networks.bench;
}
...
d.run(function () {
	var modules = [];
	console.log(figlet.textSync('    Benchcore', {
    font: 'Swamp Land',
	...
	async.auto({
		...
		connect: ['config', 'genesisblock', 'logger', 'build', 'network', function (scope, cb) {
			...
			scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
				scope.logger.info('# BenchCore node server started on: ' + scope.config.address + ':' + scope.config.port);

				if (!err) {
					if (scope.config.ssl.enabled) {
						scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
							scope.logger.info('BenchCore https connection initiated: ' + scope.config.ssl.options.address + ':' + scope.config.ssl.options.port);
	...
	vorpal
	  ...
	  .action(function(args, callback) {
	    ...
	    var passphrase = require("bip39").generateMnemonic();
			...
			self.log("WIF     - private:",require("bcorejs").crypto.getKeys(passphrase).toWIF());
			self.log("Address - public :",require("bcorejs").crypto.getAddress(require("bcorejs").crypto.getKeys(passphrase).publicKey));
			callback();
	  });
	var account=null;
	vorpal
