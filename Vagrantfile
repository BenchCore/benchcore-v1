...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision :shell, privileged: false, inline: <<-SHELL
    ...
    sudo -u postgres createdb -O $USER bench_main
    sudo service postgresql start

    git clone https://github.com/benchcore/benchcore-v1.git

    cd /home/vagrant/benchcore-v1
    #rm -fr node_modules
    npm install grunt-cli
    npm install
    forever app.js --genesis genesisBlock.bench.json --config config.bench.json
  SHELL

  config.vm.provider "virtualbox" do |v|
    v.memory = 1024
    v.customize ["modifyvm", :id, "--cpuexecutioncap", "50"]
    v.name = "benchcore_main"
  end
